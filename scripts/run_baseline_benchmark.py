...
def run_benchmark(
    ...
    logger.info(
        ...
        f"MAF={maf}, H2={heritability}, Replicates={num_replicates}"
    )
    ...
    for rep in range(num_replicates):
        ...
        # 1. Partitioned Transformer (Proposed)
        tf_res = run_full_pipeline(data, epochs=15, device=device)
        ...
        deepcombi = DeepCOMBIBaseline(num_snps=n_snps, epochs=20, device=device).fit(X, y)
...
if __name__ == "__main__":
    ...
    run_benchmark(order=args.order, model_type=args.model, num_replicates=args.replicates)
